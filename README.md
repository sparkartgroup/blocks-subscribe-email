...
### `hiddenFields`
An optional array of objects used to add `<input type="hidden">` elements to the default template to allow additional data to be submitted along with a form. As an example, on Universe forms this can be used to track an opt-in source using the [`lists_opt_in_source` parameter](https://github.com/SparkartGroupInc/sparkart-services/blob/master/doc/api/contacts.md);

```
hiddenFields: [
  {
    name: 'contact[lists_opt_in_source]',
